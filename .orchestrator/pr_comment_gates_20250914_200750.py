...
pr_num  = 15
run_id  = "20250914_200750"
gh   = Github(auth=Auth.Token(token))
repo = gh.get_repo(repo_nm)
pr   = repo.get_pull(pr_num)
data = json.loads(pathlib.Path(f"reports/gates_{run_id}.json").read_text(encoding="utf-8"))
b,t,l = data["steps"]["build"], data["steps"]["test"], data["steps"]["lint"]
