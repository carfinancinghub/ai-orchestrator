...
try {
  $prRaw = gh pr view $Pr --repo $Repo --json number,state,mergeable,headRefName,baseRefName,files
  if (-not $prRaw) { throw "Empty PR info" }
  $prObj = $prRaw | ConvertFrom-Json
  $pr = [pscustomobject]@{
    num       = $prObj.number
    state     = $prObj.state
    mergeable = $prObj.mergeable
    head      = $prObj.headRefName
    base      = $prObj.baseRefName
    files     = @($prObj.files | ForEach-Object { # scripts\upload_to_github.ps1
<# 
Upload batch artifacts summary to a PR, with optional commit/push in the same repo.
...
.path })
  ...
  throw ("Unable to get PR info for {0} #{1}. {2}" -f $Repo, $Pr, # scripts\upload_to_github.ps1
<# 
Upload batch artifacts summary to a PR, with optional commit/push in the same repo.
