...
$files = @(); $files += (git ls-files *.js) 2>$null; $files += (git ls-files *.jsx) 2>$null
$bad = $files | Where-Object { $_ -and $_ -notmatch $noisy -and $_ -notmatch $tests -and (Split-Path $_ -Leaf) -notmatch $allowJsCfg }
if ($bad -and $bad.Count) { Write-Host "❌ JS/JSX files blocked:`n$($bad -join "`n")"; exit 1 } else { Write-Host "JS/JSX gate ✅"; exit 0 }
